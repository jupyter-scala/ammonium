...
object ResolverHelpers {
  ...
  private class WarnOnOverwriteFileRepo extends FileRepository() {
  ...
  private class LocalIfFileRepo extends URLRepository {
  ...
  private class PluginCapableResolver(pattern: String) extends IBiblioResolver with ChecksumFriendlyURLResolver with DescriptorRequired {
    def setPatterns() = {
