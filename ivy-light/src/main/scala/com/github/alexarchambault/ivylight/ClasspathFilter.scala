...
// Extracted and refactored from SBT

import java.io.File
import java.net._

/** Doesn't load any classes itself, but instead verifies that all classes loaded through `parent` come from `classpath`.*/
class ClasspathFilter(parent: ClassLoader, classpath: Set[File]) extends ClassLoader(parent) {
  ...
  private def onClasspath(src: URL) =
    src == null || urlAsFile(src).exists(f => classpath(f) || directories.exists(relativize(_, f).isDefined))
