...
object ShellDisplay {
  ...
  def apply(d: CodeItem, colors: Colors): String =
    d match {
      ...
      case Identity(ident) =>
        s"""{ import $$user._; BridgeHolder.shell.term.display($$user.$ident, $$user.$ident, "$ident", _root_.scala.None) }"""
      case LazyIdentity(ident) =>
        s"""{ import $$user._; BridgeHolder.shell.term.display($$user.$ident, $$user.$ident, "$ident", _root_.scala.Some("<lazy>")) }"""
